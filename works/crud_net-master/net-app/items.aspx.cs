...
public partial class items : System.Web.UI.Page
{
    ...
    private void LoadItems()
    {
        using (var cn = new MySqlConnection(constr))
        {
            ...
            {
                using (var cmd = new MySqlCommand("SELECT * FROM app.items where is_deleted=false"))
                {
    ...
    protected void Insert(object sender, EventArgs e)
    {
        ...
        using (var cn = new MySqlConnection(constr))
        {
            using (var cmd = new MySqlCommand("INSERT INTO app.items (name,file,is_deleted,created_time,updated_time) VALUES (@Name,@File,false,current_timestamp,current_timestamp)"))
            {
    ...
    protected void OnRowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        var file = ReadAllBytes(fileUpload.PostedFile.InputStream);
            ...
            using (var cn = new MySqlConnection(constr))
            {
                using(var cmd = new MySqlCommand("UPDATE app.items SET Name = @Name, File=@File,created_time=created_time,updated_time=DEFAULT WHERE id = @id"))
                {
                    cmd.Parameters.AddWithValue("@id", id);
                    cmd.Parameters.AddWithValue("@Name", name);
                    cmd.Parameters.AddWithValue("@File", file);
